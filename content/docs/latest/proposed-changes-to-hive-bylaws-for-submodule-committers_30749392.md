...
**Note:** Such actions will also be referred to the ASF board by the PMC chair.   |  Consensus   |  Active PMC members (excluding the committer in question if a member of the PMC).   |  6  |  private@hive.apache.org  |
|  PMC Member Removal   |  When removal of a PMC member is sought.  
**Note:** Such actions will also be referred to the ASF board by the PMC chair.   |  Consensus   |  Active PMC members (excluding the member in question).   |  6  |  private@hive.apache.org  |
