...
The syntax to create a materialized view in Hive is very similar to the [CTAS statement](/docs/latest/language/languagemanual-ddl#create-table-as-select-ctas) syntax, supporting common features such as partition columns, custom storage handler, or passing table properties.
...
Materialized views can be stored in external systems, e.g., [Druid](/docs/latest/user/druid-integration), using custom storage handlers. For instance, the following statement creates a materialized view that is stored in Druid:  
**Example:**
